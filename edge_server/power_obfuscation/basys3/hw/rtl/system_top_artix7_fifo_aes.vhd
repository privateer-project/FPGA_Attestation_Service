...
LIBRARY ieee;
	...
	ARCHITECTURE behavior OF system_top IS
	  ...
	  -- RO Signals
	  signal ro_output : std_logic_vector(17 downto 0);
	  signal ro_en : std_logic;
	  signal ro_output_reg : std_logic_vector(17 downto 0);
	  ...
	  attribute keep of ro_en                  : signal is "true";
      attribute keep of ro_output              : signal is "true";
      attribute keep of ro_output_reg          : signal is "true";
	  ...
	  attribute dont_touch of ro_inst          : label is "true";
	...
	BEGIN
	  ...
	  --ro_en <= aes_bsy_s; -- Enable RO only when AES is busy
	  ro_en <= '1';
	  
	  ro_inst: entity work.RO_array
	  	generic map (
            NUM_ROS => 18
        )
		port map (
	  ...
	  process(clk_sensor)
      begin
        if rising_edge(clk_sensor) then
            if ro_en = '1' then
                ro_output_reg <= ro_output;
            end if;
        end if;
      end process;
